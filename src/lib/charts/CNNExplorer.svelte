<script>
    ...
    async function runInferenceAndGetAllActivations() {
        ...
        tf.tidy(() => { // Outer tidy scope for the entire inference and activation extraction process
            ...
            model.layers.forEach((layer, idx) => {
                ...
                try {
                    ...
                    if ((layerClassName.includes('Conv') || layerClassName.includes('Pool')) && layerOutputShape.length === 4) {
                        ...
                        for (let i = 0; i < numChannelsToDisplay; i++) {
                            ...
                            tf.tidy(() => { 
                                const singleChannelTensor = currentOutput.slice([0, 0, 0, i], [1, h, w, 1]).squeeze();
                                const resizedTensor = tf.image.resizeNearestNeighbor(singleChannelTensor.expandDims(-1), [displaySize, displaySize]);
                    ...
                    } else if (layerClassName === 'Dense' && layerOutputShape.length === 2) { // Handle Dense layers
                        tf.tidy(() => {
                    ...
                    } else if (layerClassName === 'Flatten') { // Handle Flatten layers
                        activationsForThisLayer.push({ 
                            isFlatten: true, 
                ...
                if (idx === model.layers.length - 1) {
                    tf.tidy(() => {
                        ...
                        const logits = (currentOutput.shape.length > 1 && currentOutput.shape[0] === 1) 
...
<div class="cnn-explorer-new">
    <div class="header-new">
        ...
        <div class="model-info-new">
            ...
            <select bind:value={currentIdx} on:change={(e) => pick(+e.target.value)} title="Select Input Image">
        ...
        <div class="cnn-visualization-pipeline">
            <div class="pipeline-stage input-stage">
                ...
                {#if inputPixels}
                    <div class="input-image-container">
                        <ImagesCanvas imageData={inputPixels} width={28} height={28} scale={4}/>
                    </div>
                {:else}
                    <p>Loading input...</p>
                {/if}
            ...
            {#each allLayerActivations as layerData, layerIndex (layerData.layerName)}
                <div class="pipeline-connector">
                    <svg width="40" height="100%" viewBox="0 0 20 100" preserveAspectRatio="xMidYMid meet">
                        <line x1="10" y1="0" x2="10" y2="100" stroke="#d1d5db" stroke-width="2" stroke-dasharray="4 2"/>
                        <polyline points="6,46 14,50 6,54" fill="none" stroke="#d1d5db" stroke-width="2"/>
                    </svg>
                </div>
                <div class="pipeline-stage layer-stage" class:error-stage={layerData.error}>
                    <div class="layer-header">
                        <span class="layer-name-new" title={layerData.layerName}>{layerData.layerName}</span>
                    ...
                    <div class="layer-meta">
                        ...
                        {:else}
                            <span>Output: {layerData.actualShape ? layerData.actualShape.slice(1).join(' × ') : 'N/A'}</span>
                    ...
                    {#if layerData.activations && !layerData.error}
                        {#if layerData.activations.length > 0 && layerData.activations[0].isDense}
                            <div class="dense-activations-container">
                                <p class="dense-info">Dense Activations ({layerData.activations[0].originalShape[0]} neurons)</p>
                                <div class="dense-values">
                                    {#each layerData.activations[0].pixels.slice(0, 10) as val, i (i)}
                                        <span class="dense-value" title={`Neuron ${i}: ${val.toFixed(3)}`}>{val.toFixed(2)}</span>
                                    {/each}
                        ...
                        {:else if layerData.activations.length > 0 && layerData.activations[0].isFlatten}
                            <div class="flatten-layer-info">
                                <p>Flatten Layer</p>
                                <span>Output Dim: {layerData.activations[0].originalShape.join(' × ')}</span>
                            </div>
                        {:else if layerData.activations.length > 0}
                            <div class="activation-maps-grid-new">
                                {#each layerData.activations as activation (activation.channelIndex)}
                                    <div class="activation-map-item" title={`Channel ${activation.channelIndex} (Original: ${activation.originalShape.join('×')})`}>
                                        <ImagesCanvas 
                                            imageData={activation.pixels} 
                                            width={activation.displaySize} 
                                            height={activation.displaySize} 
                                            scale={1.5} />
                                        <span class="map-channel-label">Ch {activation.channelIndex}</span>
                                    </div>
                                {/each}
                            </div>
                        {:else if !layerData.error}
                            <p class="no-viz-placeholder">No specific visualization (e.g., {layerData.layerType})</p>
                        {/if}
                    {/if}
                </div>
            {/each}
            ...
            <div class="pipeline-stage output-stage">
                ...
                {#if predicted !== null && imagesMeta[currentIdx]}
                    <div class="prediction-badge-new">
                        Predicted: <strong>{predicted}</strong> (Actual: {imagesMeta[currentIdx]?.label})
                    </div>
                {/if}
                {#if probs}
...
<style>
    ...
    .pipeline-stage.error-stage { /* Style for layers that encountered an error */
        border-color: #f87171; 
    ...
    .pipeline-stage h3, .layer-header {
        margin: 0 0 5px 0;
        color: #1f2937;
        font-size: 16px;
        font-weight: 600;
    ...
    .layer-header { /* Specific styling for layer headers */
        display: flex;
        justify-content: space-between;
        align-items: center;
    }
    .layer-header .layer-name-new { /* Layer name might be long */
        font-size: 15px;
        white-space: nowrap;
        overflow: hidden;
    ...
    .layer-type-badge { /* Small badge for layer type (Conv2D, Dense, etc.) */
    ...
    .input-image-container canvas { /* Style the canvas within */
        border: 1px solid #d1d5db;
    ...
    .activation-map-item { /* Container for one activation map + its label */
        display: flex;
        flex-direction: column;
        align-items: center;
    ...
    .map-channel-label { /* Label for the channel (e.g., "Ch 0") */
        font-size: 10px;
        color: #4b5563;
        margin-top: 4px;
    }
    ...
    .dense-values { /* Container for neuron values */
        display: flex;
        flex-wrap: wrap; /* Allow values to wrap to next line */
        gap: 5px;
    }
    .dense-value { /* Individual neuron value badge */
    ...
    .pipeline-connector svg line, .pipeline-connector svg polyline {
    ...
    .prediction-badge-new { /* Badge for the predicted class */
     ...
     .output-chart-container { /* Container for the PredictionChart component */
        margin-top: 10px;
        width: 100%; 
